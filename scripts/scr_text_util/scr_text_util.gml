	...
	for (var saved_pos = 1; pos <= len; pos += 1)
	{
		...
		switch char
		{
			...
			case ord("{"):
				...
				array_push(arr, [char_x - offset.x, char_y - offset.y, texttype.array, te, n[0]]); // 2 << 0
				pos = n[1];
				char_x = n[2];
				char_y = n[3];
				break;
			...
			default:
				...
				array_push(arr, [char_x - offset.x, char_y - offset.y, texttype.normal, n]);
			...
			case texttype.icon:
				...
				if icon != -4
				{
					...
					if icon.str != ""
					{
						...
						var o = font_get_offset();
						draw_text_color(cx + 16 - (o.x / 2), cy + 14 - (o.y / 2), icon.str, c_black, c_black, c_black, c_black, alpha);
						draw_set_font(f);
